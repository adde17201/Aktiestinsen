...
def analyze_stock(ticker):
    try:
        ...
        # Tvinga till 1D-array
        close = df['Close'].astype(float).values.flatten()
        high = df['High'].astype(float).values.flatten()
        low = df['Low'].astype(float).values.flatten()
        volume = df['Volume'].astype(float).values.flatten()

        df['EMA9'] = ta.trend.EMAIndicator(close=pd.Series(close), window=9).ema_indicator()
        df['EMA50'] = ta.trend.EMAIndicator(close=pd.Series(close), window=50).ema_indicator()
        df['RSI'] = ta.momentum.RSIIndicator(close=pd.Series(close)).rsi()
        df['MACD'] = ta.trend.macd_diff(pd.Series(close))
